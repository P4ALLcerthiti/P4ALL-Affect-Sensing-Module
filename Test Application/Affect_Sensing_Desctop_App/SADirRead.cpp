...
#ifdef _UNICODE
			 USES_CONVERSION;
			 if ((strcmp(W2A(FindFileData.cFileName), ".")!=0) && (strcmp(W2A(FindFileData.cFileName), "..")!=0)) 
	#else
...
#ifdef _UNICODE
		USES_CONVERSION;
		if (!FindFiles(curDir, A2W(pFilemask), bIncludeFilesInFileList, bIncludeFoldersInFileList))
	#else
...
#ifdef _UNICODE
	USES_CONVERSION;
	if (bIsFolder && (strcmp(W2A(FindFileData.cFileName), ".")==0) || (strcmp(W2A(FindFileData.cFileName), "..")==0))		
#else
